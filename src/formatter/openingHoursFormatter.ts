...
import { FormattedPlaceDetails, OpeningHoursRow } from '../types'
...
export const buildOpeningHours = (
  ...
  openHours.forEach((o) => {
    if (o.open === '00:00' && o.close === '23:59') {
      ...
      formatted[o.day].push(
        dayjs(o.open, 'HH:mm').format(format) +
          ' - ' +
          dayjs(o.close, 'HH:mm').format(format)
      )
    }
  })
...
export const buildCurrentOpeningStatus = (
  placeDetails: FormattedPlaceDetails,
  language: string,
  uses24: boolean = true
): string => {
  return getCurrentOpeningState(
...
const getFormattedHours = (minuts: number, uses24: boolean = true): string => {
  const date = new Date()
...
const dayOfWeekAsString = (dayIndex: number, language: string): string => {
  return (
    [
      lang_sunday(language),
      lang_monday(language),
      lang_tuesday(language),
      lang_wednesday(language),
      lang_thursday(language),
      lang_friday(language),
      lang_saturday(language),
    ][dayIndex] || ''
  )
}

const getCurrentOpeningState = (
  input: any[],
  timezoneCode: string,
  language: string,
  uses24?: boolean
  ...
  const currentMinuts =
    (new Date().getHours() + diff) * 60 + new Date().getMinutes()

  const intervals = sortInterval(
    buildIntervals(input),
    new Date().getDay(),
    currentMinuts
  )

  const currentInterval = getIntervalIfOpenFromIntervals(
    intervals,
    weekday,
    currentMinuts
  )
  if (currentInterval) {
    const closingInterval = getNextCloseIntervalFromIntervals(
      intervals,
      currentInterval
    )
    if (closingInterval) {
      if (closingInterval === currentInterval) {
        return lang_open_close_at(
          language,
          getFormattedHours(closingInterval.close, uses24)
        )
      }
      if (
        closingInterval.day - currentInterval.day === 1 ||
        closingInterval.day - currentInterval.day === -6
      ) {
        return lang_open_close_tomorrow(
          language,
          getFormattedHours(closingInterval.close, uses24)
        )
      } else {
        return lang_open_close(
          language,
          dayOfWeekAsString(closingInterval.day, language),
          getFormattedHours(closingInterval.close, uses24)
        )
      }
    } else {
    ...
    const openingInterval = getNextOpeningIntervalFromIntervals(
      intervals,
      weekday,
      currentMinuts
    )
    if (openingInterval) {
      if (openingInterval.day === weekday) {
        return lang_close_open_at(
          language,
          getFormattedHours(openingInterval.open, uses24)
        )
      }
      if (
        openingInterval.day - weekday === 1 ||
        openingInterval.day - weekday === -6
      ) {
        return lang_close_open_tomorrow(
          language,
          getFormattedHours(openingInterval.open, uses24)
        )
      } else {
        return lang_close_open(
          language,
          dayOfWeekAsString(openingInterval.day, language),
          getFormattedHours(openingInterval.open, uses24)
        )
      }
    } else {
...
const getNextOpeningIntervalFromIntervals = (
  intervals: any[],
  day: number,
  minuts: number
): any => {
  ...
  for (let i = 0; i < intervals.length; i++) {
    if (
      intervals[i].day === day &&
      intervals[i].open > minuts &&
      intervals[i].close > minuts
    ) {
      return intervals[i]
    }
...
const getNextCloseIntervalFromIntervals = (
  intervals: any[],
  current: any
): any => {
...
const getIntervalIfOpenFromIntervals = (
  intervals: any[],
  day: number,
  minuts: number
): any => {
  for (let i = 0; i < intervals.length; i++) {
    if (
      day === intervals[i].day &&
      minuts > intervals[i].open &&
      minuts < intervals[i].close
    ) {
      return intervals[i]
    }
  }
